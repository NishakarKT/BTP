...
const Product = () => {
  ...
  const { cart, wishlist, product, setProduct } = useContext(HomeContext);
    ...
    product ? <Stack direction="row">
      <Stack flex={{ xs: 0, sm: 1 }}>
        <Carousel showThumbs={false} stopOnHover={false} autoPlay infiniteLoop>
          {product.files.map(file => <img key={file} style={{ width: "100%", height: "100vh", objectFit: "cover" }} src={file} alt="" loading="lazy" />)}
        </Carousel>
      </Stack>
      <Stack flex={2} style={{ width: "100%", height: "100vh", overflowX: "hidden", overflowY: "auto" }}>
        <Card elevation={0} sx={{ overflowY: "auto", height: "100vh", overflowX: "hidden", '&::-webkit-scrollbar': { width: "5px" }, '&::-webkit-scrollbar-thumb': { backgroundColor: "lightgray" } }}>
          ...
          } avatar={<Avatar src={product.ownerProfilePic} loading="lazy" />} title={product.title} subheader={product.ownerName} align="left" />
          <Stack display={{ xs: "block", sm: "none" }}>
            <Carousel showThumbs={false} stopOnHover={false} autoPlay infiniteLoop>
              {product.files.map(file => <img key={file} style={{ width: "100%", height: "300px", objectFit: "cover" }} src={file} alt="" loading="lazy" />)}
            </Carousel>
          </Stack>
