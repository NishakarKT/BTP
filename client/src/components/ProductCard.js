...
const ProductCard = ({ product, sx }) => {
    ...
    return (
        <Card sx={{ ...sx, position: "relative" }}>
            ...
            <Stack sx={{ height: "100%" }}>
                <CardMedia onClick={() => setProduct(product)} sx={{ cursor: "pointer" }} component="img" height="194" image={product.files.length ? product.files[0] : ""} alt="" loading="lazy" />
                <CardHeader onClick={() => setProduct(product)} sx={{ cursor: "pointer" }} avatar={<Avatar src={product.ownerProfilePic} loading="lazy" />} title={truncateStr(product.title, 25)} subheader={truncateStr(product.ownerName, 25)} align="left" />
